...
\todo{Capture d'écran : \code{kubectl get pods,svc -n eventhub} montrant \code{event-service} en \code{Running}.}
...
\todo{Capture d'écran : \code{kubectl get ingress -n eventhub} et navigateur sur \code{http://eventhub.local}.}
...
\todo{Capture d'écran : requête \code{curl} vers \code{/users/auth/login/} puis \code{/events/} avec le token obtenu.}
...
\todo{Capture d'écran : \code{kubectl exec postgres-0 -n eventhub -- psql -U postgres -l} montrant les deux bases.}
...
\todo{Capture d'écran : \code{kubectl get secret,role,rolebinding,sa -n eventhub}.}
...
\todo{Captures d'écran individuelles des Google labs (une par membre du trinôme) à joindre au courriel de rendu, conformément à la consigne du sujet.}
...
\todo{Capture d'écran : \code{kubectl get all -n eventhub} avec tous les objets \code{Running} / \code{Ready}.}

\todo{Capture d'écran : page d'accueil du frontend après connexion, avec la liste des évènements chargée depuis l'API.}
